...
const suggestionList:HTMLUListElement = document.getElementById("suggestions") as HTMLUListElement
...
function postAndUpdate(): void {
  ...
  fetch("http://localhost:4567/results", {
    ...
    headers: {
      ...
      .then((response) => response.json()).then((data: Matches) => {updateSuggestions(data.matches)})
...
document.addEventListener("keyup", async (keyboardEvent: KeyboardEvent) => {
  if (keyboardEvent.key == "s") {
    await updateValues("Taurus", "Virgo", "Aquarius")
    postAndUpdate()
  }
})
